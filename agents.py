...
# website_url='https://aminutewithmary.com/'
...
class MarketingAnalysisAgents:
	...
	def product_competitor_agent(self):
		return Agent(
			...
			tools=[
					...
					SearchTools.search_internet,selenium_tool
			],
	...
	def strategy_planner_agent(self):
		return Agent(
			...
			tools=[
					...
					SearchTools.search_instagram,selenium_tool
			],
			llm=self.OpenAIGPT35,
			verbose=True
		)
	...
	def senior_photographer_agent(self):
		return Agent(
				...
				goal=dedent("""\
					Make the most amazing designs for instagram ads that
					capture emotions and convey a compelling message."""),
				backstory=dedent("""\
					As a Senior Photographer at a leading digital marketing
					agency, you are an expert at creating amazing designs that
					inspire and engage, you're now working on a new campaign for a super
					important customer and you need to take the most amazing designs."""),
				tools=[
					# BrowserTools.scrape_and_summarize_website,
					SearchTools.search_internet,
					SearchTools.search_instagram,selenium_tool
